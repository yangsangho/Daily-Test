...
class BarChart : View
{
    ...
    init{
        textPaint.apply {
            ...
            textSize = resources.getDimensionPixelSize(R.dimen.noItemFontSize).toFloat()    // resources를 static 쪽에서 사용 못함
        }
    
        val drawable = ContextCompat.getDrawable(context, R.drawable.ic_round_number1)  // context를 static 쪽에서 사용 못함
        bitmap = drawable?.toBitmap(100,100)
    }
    ...
    private fun makeDesiredTextPaint(desiredWidth: Float)
    {
        ...
        barDescFontPaint.first.getTextBounds(baseTextSizeStr.first, 0, baseTextSizeStr.first.length, bounds)
        ...
        barDescFontPaint.second.getTextBounds(baseTextSizeStr.second, 0, baseTextSizeStr.second.length, bounds)
        desiredTextSize = testTextSize * desiredWidth / bounds.width()
